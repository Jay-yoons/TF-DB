...
public class BookingController {
    ...
    public List<BookingListResponse> findBookingList(@PathVariable String userId) {
        // 토큰에서 추출한 userId와 경로의 userId가 일치하는지 검증하는 로직 추가
        String currentUserId = getCurrentUserIdFromToken();
        if (currentUserId == null || !currentUserId.equals(userId)) {
            log.error("권한이 없는 사용자입니다: currentUserId={}, pathUserId={}", currentUserId, userId);
    ...
    private String getCurrentUserIdFromToken() {
        ...
        try {
            ...
            // 'username' 클레임을 읽도록 수정
            Object username = payloadMap.get("username");
            if (username instanceof String) {
                return (String) username;
            } else {
                log.error("'username' 키를 찾을 수 없거나 String 타입이 아닙니다.");
                return null;
            }
